...
class BaseLine(nn.Module):
    def __init__(self, group=1):
        ...
        self.encoder1 = encoder_block(3, 6, groups=group)

        # [b,6,320,240] ==> [b,12,160,120]
        self.encoder2 = encoder_block(6, 12, groups=group)

        # [b,12,160,120] ==> [b,24,80,60]
        self.encoder3 = encoder_block(12, 24, groups=group)

        # [b,24,80,60] ==> [b,48,40,30]
        self.encoder4 = encoder_block(24, 48, groups=group)
    ...
    def forward(self, x):
        ...
        decoder2 = self.decoder2(torch.cat([encoder4, decoder1], 1))  # [b,48,40,30]+[b,48,40,30] ==> [b,96,40,30] ==> [b,24,80,60]
        decoder3 = self.decoder3(torch.cat([encoder3, decoder2], 1))  # [b,24,80,60]+[b,24,80,60] ==> [b,48,80,60] ==> [b,12,160,120]
        decoder4 = self.decoder4(torch.cat([encoder2, decoder3], 1))  # [b,12,160,120]+[b,12,160,120] ==> [b,24,160,120] ==> [b,6,320,240]

        output = self.output(torch.cat([encoder1, decoder4], 1))  # [b,6,320,240]+[b,6,320,240] ==> [b,12,320,240] ==> [b,1,640,480]
